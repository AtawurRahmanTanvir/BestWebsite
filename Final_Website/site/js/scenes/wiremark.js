...
import { THREE, makeRenderer, disposeAll, buildMark, edgesOf, damp, smooth, makeEnvironment, markSilhouette } from './util.js';
...
export async function create({ canvas, quality, env, width, height, host }) {
  ...
  // owner note 15: the wire must be the OFFICIAL mark — extrude its exact alpha silhouette
  try {
    const sil = await markSilhouette({ depth: .34 });
    const ch = mark.getObjectByName('chevron');
    if (ch && sil) { ch.geometry.dispose && ch.geometry.dispose(); ch.geometry = sil; ch.position.set(0, .3, 0); ch.scale.setScalar(1.35); }
  } catch (err) { console.warn('[wiremark] silhouette fallback to engineered mark', err); }
