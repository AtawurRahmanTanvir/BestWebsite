...
export async function create({ canvas, quality, env, width, height }) {
  ...
  const metal = new THREE.MeshStandardMaterial({ color: 0x1a1a1d, metalness: .95, roughness: .35 });
  const edgeMat = new THREE.MeshBasicMaterial({ color: 0xff2438, transparent: true, opacity: .9, blending: THREE.AdditiveBlending, depthWrite: false });
