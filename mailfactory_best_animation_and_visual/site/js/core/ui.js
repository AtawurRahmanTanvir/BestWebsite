...
function initCursor() {
  ...
  const c = document.createElement('div'); c.className = 'cursor is-hidden'; c.innerHTML = '<div class="ring"><span></span></div><div class="dot"></div>'; document.body.appendChild(c);
  const ring = c.firstChild; const dot = c.lastChild; const labelEl = ring.firstChild;
  let x = -100, y = -100, rx = -100, ry = -100, shown = false;
  addEventListener('pointermove', (e) => { x = e.clientX; y = e.clientY; if (!shown) { shown = true; c.classList.remove('is-hidden'); rx = x; ry = y; } }, { passive: true });
  document.addEventListener('pointerleave', () => c.classList.add('is-hidden'));
  document.addEventListener('pointerenter', () => shown && c.classList.remove('is-hidden'));
  addEventListener('pointerdown', () => c.classList.add('is-down')); addEventListener('pointerup', () => c.classList.remove('is-down'));
  gsap.ticker.add((t, dt) => { const k = 1 - Math.exp(-dt / 1000 * 22); rx += (x - rx) * k; ry += (y - ry) * k; dot.style.transform = `translate(${x}px,${y}px)`; ring.style.transform = `translate(${rx}px,${ry}px)`; });
  document.addEventListener('pointerover', (e) => {
    const t = e.target.closest('[data-cursor]'); const link = e.target.closest('a,button,[role="button"],input,textarea,label,select');
    c.classList.toggle('is-label', !!t); c.classList.toggle('is-link', !t && !!link);
    if (t) labelEl.textContent = t.dataset.cursor;
  });
  document.addEventListener('pointerout', (e) => { if (e.relatedTarget && (e.relatedTarget.closest('[data-cursor]') || e.relatedTarget.closest('a,button'))) return; c.classList.remove('is-label', 'is-link'); });
  document.addEventListener('mf:iframe-hover', (e) => c.classList.toggle('is-hidden', !!e.detail));
  // let the cursor sit inside the ring's box: fix ring transform origin
  ring.style.left = '0'; ring.style.top = '0'; ring.style.marginLeft = '-20px'; ring.style.marginTop = '-20px';
  dot.style.left = '0'; dot.style.top = '0'; dot.style.marginLeft = '-4px'; dot.style.marginTop = '-4px';
}
...
export function initUI() {
  buildRail(); buildMenu(); initNav(); initDownload(); initShare(); initCursor(); initMagnetic();
}
export { $, $$ };
